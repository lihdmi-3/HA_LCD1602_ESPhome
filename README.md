...
设备启动时，last_merged_uptime 被设置为当前 uptime_sec（整数秒）。

每隔 10 分钟，计算 delta = current_uptime - last_merged_uptime，将 delta 累加到 total_uptime_base，并更新 last_merged_uptime。

传感器 total_uptime_sec 实时返回 total_uptime_base + (current_uptime - last_merged_uptime)，确保显示值连续。

断电重启后，total_uptime_base 恢复，last_merged_uptime 重置为当前 uptime，继续累加。
...
配置修改指南
修改实体 ID：在 sensor: 部分，将所有 entity_id 替换为你 Home Assistant 中对应的实体。

调整 I2C 地址：如果 LCD 不显示，尝试将 address: 0x27 改为 0x3F。

背光极性：若背光逻辑相反，将 inverted: false 改为 true。

WiFi 凭据：在 secrets.yaml 中定义 wifi_ssid 和 wifi_password。
